...
public class LinkedHashSetMultiValueMap<K, V> implements SetMultiValueMap<K, V>, Serializable {

	private static final long serialVersionUID = 1L;

	private final Map<K, Set<V>> targetMap;

	public LinkedHashSetMultiValueMap() {
		this.targetMap = new LinkedHashMap<K, Set<V>>();
	}

	public LinkedHashSetMultiValueMap(int initialCapacity) {
		this.targetMap = new LinkedHashMap<K, Set<V>>(initialCapacity);
	}

	public LinkedHashSetMultiValueMap(Map<K, Set<V>> otherMap) {
	...
	@Override
	public LinkedHashSetMultiValueMap<K, V> clone() {
		return new LinkedHashSetMultiValueMap<K, V>(this);
	}

	public LinkedHashSetMultiValueMap<K, V> deepCopy() {
		LinkedHashSetMultiValueMap<K, V> copy = new LinkedHashSetMultiValueMap<K, V>(this.targetMap.size());
		for (Map.Entry<K, Set<V>> entry : this.targetMap.entrySet()) {
			copy.put(entry.getKey(), new LinkedHashSet<V>(entry.getValue()));
		}
		return copy;
	}
