 ...
 * <p>
 * 关于使用内置拦截器的注意事项：<br>
 * 1、在控制器类级别上定义拦截服务或拦截器注解将适用于拦截该控制器类的所有映射方法（即含有@Render注解的方法）。
 * 2、在控制器类内方法互相调用的情况，如果调用没有@Render注解的其他方法，则不会触发任何拦截服务；如果调用含有@Render
 * 注解的其他方法，此时需要看具体情况了，如果被调用的方法已经响应过映射请求，则会触发该方法上对应的拦截服务，如果被调
 * 用的方法从未响应过映射请求，则不会触发该方法上对应的拦截服务（因为拦截服务只有在第一次响应映射请求的时候才放入缓存，
 * 当该方法从未响应过映射请求的情况下被其他方法调用时，因为找不到相应的拦截服务缓存，所以不会触发对应的拦截服务，也可以
 * 看做是本框架设计中存在的Bug，如果启用了本框架内置拦截器的情况，在开发过程中应避免在控制器类内互相调用含有@Render注
 * 解的方法，以避免可能产生触发拦截服务不一致的情况发生）。
 *
 */
