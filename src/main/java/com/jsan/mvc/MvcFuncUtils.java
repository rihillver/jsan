...
public class MvcFuncUtils {
    ...
    public static void addPackageClassToMap(String packageName, Map<String, Class<?>> classMap, boolean qualifiedClassNameToLowerCase) {
	...
	try {
		...
		if ("file".equals(protocol)) { // file 类型的扫描
		    ...
		    addPackageClassToMapByFile(packageName, packagePath, recursive, classMap, qualifiedClassNameToLowerCase);
			...
			while (entries.hasMoreElements()) {
				...
				if ((idx != -1) || recursive) {
					...
					try {
					    ...
					    classMap.put(qualifiedClassNameToLowerCase ? qualifiedClassName.toLowerCase() : qualifiedClassName, Thread.currentThread().getContextClassLoader().loadClass(qualifiedClassName)); // 此处判断类名是否转换为小写作为key
    ...
    private static void addPackageClassToMapByFile(String packageName, String packagePath, final boolean recursive, Map<String, Class<?>> classMap, boolean qualifiedClassNameToLowerCase) {
	...
	for (File file : dirfiles) {
	    ...
	    if (file.isDirectory()) {
		addPackageClassToMapByFile(packageName + "." + file.getName(), file.getAbsolutePath(), recursive, classMap, qualifiedClassNameToLowerCase);
	    } else {
